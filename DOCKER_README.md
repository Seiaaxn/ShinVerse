# Docker Setup Guide - ShinVerse

Panduan untuk menjalankan aplikasi ShinVerse menggunakan Docker.
...
1. **Frontend** (web) - React/Vite app dengan Nginx
   - Port: 8072
   - Container: ShinVerse-frontend

2. **Backend** (backend) - Hono API Server dengan SQLite
   - Port: 8062
   - Container: ShinVerse-backend
...
```bash
# Backend shell
docker exec -it ShinVerse-backend sh

# Frontend shell
docker exec -it ShinVerse-frontend sh
```
...
docker cp ShinVerse-backend:/app/server/statistics.db ./backup-statistics.db
...
docker cp ./backup-statistics.db ShinVerse-backend:/app/server/statistics.db
...
docker stats ShinVerse-backend ShinVerse-frontend

# View detailed info
docker inspect ShinVerse-backend
```
